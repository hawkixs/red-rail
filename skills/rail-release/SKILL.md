---
name: rail-release
description: Cut a release of a prod-tier ReD project with `rail release --version X.Y.Z` — image built on the host and pushed to GHCR by digest, annotated tag on `origin` (and on a declared mirror), `released` attestation — then open the receipts PR. Use from the host, on main, after the integration receipt.
---
